...
struct CameraUniform {
    // cmake를 bytemuck에서 직접 쓸 수 없음. 그래서 f32 array로 변환해야 함
    view_proj: [[f32; 4]; 4],
}
...
impl DepthPass {
    ...
    fn new(device: &wgpu::Device, config: &wgpu::SurfaceConfiguration) -> Self {
        ...
        let vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            contents: bytemuck::cast_slice(DepthPass::DEPTH_VERTICES),
            usage: wgpu::BufferUsages::VERTEX,
        });

        let index_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Depth Pass Index Buffer"),
            contents: bytemuck::cast_slice(DepthPass::DEPTH_INDICES),
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("depth_render.wgsl").into()),
        });
        ...
        Self {
            ...
            vertex_buffer,
            index_buffer,
            num_depth_indices: DepthPass::DEPTH_INDICES.len() as u32,
            render_pipeline,
        }
    }
...
impl State {
    ...
    pub async fn new(window: Arc<Window>) -> anyhow::Result<Self> {
        ...
        // BindGroupLayout를 이용하여 BindGroup생성
        // BindGroup이 BindLayout의 더 specific한데 분리된 이유는 같은 layout이면 BindGroup을 swap할 수 있기 때문
        // 만들 texture와 smapler는 BindGroup에 추가되어야 함
